...
*GROUP 1*
https://chat.whatsapp.com/DYp7FwfL9xsKURihcdT0l8
...
case 'repo': case 'botrepo':
    ...
    reply(`*pickachu's bot script😘:* https://github.com/Ayush-pandey-u/pickachu-MD`)
    break
...
await Miku.sendMessage(m.chat, { delete: key })
 ...
 let teks = ` 「  pickachu's pm user list  」\n\nTotal ${anu.length} users are using chikuin personal chat.`
 for (let i of anu) {
 ...
 let teks = ` 「  pickachu's group user list  」\n\nTotal ${anu.length} users are using bot in Groups.`
 for (let i of anu) {
...
if (isBanChat) return reply(mess.bangc)
 ...
 if (!m.isGroup) return replay(mess.grouponly)
 if (!isAdmins && !isCreator) return replay(mess.useradmin)
 let teks = `「 _chikune tag nahi kiya sachi muchi_ 」
...
case 'public': {
 ...
 Miku.public = true
 reply('chikuis  Publicly accessable!')
...
case 'command': case 'listmenu' : case 'list': case 'l' : {
	const sections = [{
								"title": "Initial Features Of chikuBot 🛰🚀",
								"rows": [
									{
										"title": "Other ☕",
							...
							},
							{
								...
								"rows": [
									     ...
									     },
									{
										...
										{
											"title": "chikusupport 🌐",
										"description": "Displays The support link of pickachu",
										"rowId": `${prefix}developer`
										}
								]
							},
							{
								"title": "about chikubot 🌝",
								"rows": [
									{
										"title": "chikubot developer",
										"description": "Displays The pickachu-MD developer",
										"rowId": `${prefix}developer`
									}
								]
							},
...
I am *pickachu*, created by Ayush pandey
...
case 'stupid':
	...
	if (isBanChat) return reply(mess.banChat)
            ...
            let ments = [me, jodoh]
            let buttons = [
                        { buttonId: '.funmenu', buttonText: { displayText: 'more fun' }, type: 1 }
                    ]
                    await Miku.sendButtonText(m.chat, buttons, jawab, pushname, m, {mentions: ments})
            }
            break
...
case  'rating' : {
  ...
  from, 
  {
   text: "contact to my owner: https://api.whatsapp.com/send?phone=+919006765023",
   footer: `${BotName}`,
   title: "CHECK THE ......",
   title: `
hey 🥵: ${pushname}
...
I am *pickachu*, a bot  deploy by 🦋⃟≛⃝𝐀𝐘𝐔𝐒𝐇⃟≛⃝
...
const txt = `Do you love pickachu? Then we are the same.`
const mikuarray= [
